...
while(c!=1)
{
		...
		if(flag==j)
		{
			cout<<"1)Deposit\t2)withdrawl\t3)Balance4)bank quiery\t5)My profile\t6)update7)Exit";
			int ji;
			cin>>ji;
			if(ji==1)
			{
			...
			else if(ji==2)
			{
				...
				int d;
				cin>>d;
				if(hi[j].amount>=1000 && hi[j].amount>d)
				{
				hi[j].amount=hi[j].amount-d;
				bk.with=bk.with+d;
				bk.balance-=d;
			...
			{
				cout<<"\nBalance :"<<hi[j].amount;//1 10000 1 name add 1000 123
			}
			else if(ji==4)
			{
				display(bk);
...
cout<<"HI\n";
			...
			{
				cout<<"What do u want to update1)Name\t2)adress\t3)Contact\t4)Exit\n";
				int change;
				cin>>change;
				if(change==1)
				{
					cout<<"Enetr the new name :";
					cin>>hi[j].name;
				}
				else if(change==2)
				{
					cout<<"\nEnetr the new adress :";
					cin>>hi[j].address;
				}
				else if(change==3)
				{
					cout<<"\nEnetr the new contact no";
					cin>>hi[j].contact;
				}
				else if(change==4)
				{
